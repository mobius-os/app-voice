...
// Downsample the recording to the clone rate with anti-aliasing, matching the
// reference pipeline (scipy resample_poly). OfflineAudioContext is the browser's
// high-quality, anti-aliased resampler — the same audio stack that records and
// plays the clone. A naive linear resample aliases sibilants into a harsh, "not
// me" clone, so there is deliberately no fallback that could silently reintroduce
// that; if Web Audio is somehow missing we fail loudly.
async function resampleMono(samples, sourceRate, targetRate = CLONE_SAMPLE_RATE) {
  ...
  if (!OfflineCtx) throw invalidCloneAudioError()
